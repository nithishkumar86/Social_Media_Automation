...
The Short is always a video and must be exactly 0 to 12 seconds, vertical (9:16), and contain no animation.
...
- **Topic** — from the heading `# [Topic] — YouTube Short (0-12s)`.
- **Concept** — the one-line idea.
- **Shot breakdown** — the 0-4s, 4-8s, and 8-12s shots.
...
3. Call `blotato_create_visual` with:
   ...
   - `inputs`: `{ "aspectRatio": "9:16", "durationSeconds": 12 }` (vertical, capped at 12 seconds; let Blotato fill the rest from the prompt)
   - `render`: true
...
7. Confirm the generated video is vertical, 0 to 12 seconds long, real footage, and contains no animation.
...
- [ ] Video is 0 to 12 seconds long and vertical (9:16) so YouTube classifies it as a Short
...
3. If the call returns a `postSubmissionId` instead of a published result (it timed out), poll `blotato_get_post_status` with that `postSubmissionId` until status is `published`. Wait at least 12 seconds between polls.
...
- The Short is always a video, exactly 0 to 12 seconds, and vertical (9:16).
...
- Ensure the post classifies as a Short — vertical video, 60 seconds or less (here, 0 to 12 seconds).
